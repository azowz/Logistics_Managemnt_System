...
class InsurancePolicy(TimestampMixin, AuditMixin, SoftDeleteMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "tenant_id", "policy_number", name="uq_insurance_policies_tenant_id_policy_number"
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            "coverage_amount IS NULL OR coverage_amount >= 0", name="coverage_amount_non_negative"
        ),
        CheckConstraint(
            "deductible_amount IS NULL OR deductible_amount >= 0", name="deductible_non_negative"
        ),
    ...
    policy_type: Mapped[InsurancePolicyType] = mapped_column(
        SAEnum(InsurancePolicyType, native_enum=False, length=32, values_callable=_enum_values),
        nullable=False,
    )
    ...
    covers_equipment: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default="false"
    )
    covers_shipment: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default="false"
    )
    covers_third_party: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default="false"
    )
    covers_hazardous_cargo: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default="false"
    )
...
class CoverageRule(TimestampMixin, AuditMixin, SoftDeleteMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "max_coverage_amount IS NULL OR max_coverage_amount >= 0",
            name="max_coverage_non_negative",
        ),
    )
    ...
    coverage_type: Mapped[CoverageType] = mapped_column(
        SAEnum(CoverageType, native_enum=False, length=32, values_callable=_enum_values),
        nullable=False,
    )
...
class Claim(TimestampMixin, AuditMixin, SoftDeleteMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "claimed_amount IS NULL OR claimed_amount >= 0", name="claimed_amount_non_negative"
        ),
        CheckConstraint(
            "approved_amount IS NULL OR approved_amount >= 0", name="approved_amount_non_negative"
        ),
        CheckConstraint("currency_code ~ '^[A-Z]{3}$'", name="currency_code"),
    )
    ...
    claim_type: Mapped[ClaimType] = mapped_column(
        SAEnum(ClaimType, native_enum=False, length=32, values_callable=_enum_values),
        nullable=False,
    )
...
class DamageReport(TimestampMixin, AuditMixin, SoftDeleteMixin, Base):
    ...
    damage_type: Mapped[DamageType] = mapped_column(
        SAEnum(DamageType, native_enum=False, length=32, values_callable=_enum_values),
        nullable=False,
    )
    severity: Mapped[ClaimSeverity] = mapped_column(
        SAEnum(ClaimSeverity, native_enum=False, length=32, values_callable=_enum_values),
...
class LiabilityRecord(TimestampMixin, AuditMixin, SoftDeleteMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="liability_amount_non_negative",
        ),
    )
    ...
    responsible_party_type: Mapped[ResponsiblePartyType] = mapped_column(
        SAEnum(ResponsiblePartyType, native_enum=False, length=32, values_callable=_enum_values),
        nullable=False,
    )
