...
class ClaimsService:
    ...
    def validate_claim_references(self, tenant_id, data) -> None:
        if data.get("policy_id") is not None:
            self._owned(self._policies.get_by_id(data["policy_id"]), tenant_id, "Policy", data["policy_id"])
        ...
        if data.get("order_id") is not None:
            self._owned(self._orders.get_by_id(data["order_id"]), tenant_id, "Order", data["order_id"])
        ...
        if data.get("compliance_check_id") is not None:
            self._owned(self._checks.get_by_id(data["compliance_check_id"]), tenant_id,
                        "Compliance check", data["compliance_check_id"])
        if data.get("permit_id") is not None:
            self._owned(self._permits.get_by_id(data["permit_id"]), tenant_id, "Permit", data["permit_id"])
    ...
    def validate_policy_coverage(self, claim: Claim) -> None:
        ...
        if not PolicyStateMachine.can_cover(policy.status):
            raise ValidationError(f"Policy is '{policy.status.value}', not active; cannot approve claim.")
    ...
    def validate_liability_distribution(self, claim_id, new_percentage, *, allow_override=False) -> None:
    ...
    def create_claim(self, *, claim_number: Optional[str] = None, **data) -> Claim:
        ...
        if data.get("claim_type") == ClaimType.EQUIPMENT_DAMAGE and data.get("equipment_id") is None:
        ...
        claim = self._claims.create(
            tenant_id=tenant_id, claim_number=number, status=ClaimStatus.CREATED,
            reported_at=utcnow(), created_by=actor_id, updated_by=actor_id, **data,
        )
        self._session.flush()
        self._emit(
        ...
        if claim.shipment_id is not None:
            self._emit(ClaimLinkedToShipment(claim_id=claim.id, tenant_id=tenant_id, shipment_id=claim.shipment_id),
                       aggregate_id=claim.id, aggregate_type="Claim", tenant_id=tenant_id)
        if claim.equipment_id is not None:
            self._emit(ClaimLinkedToEquipment(claim_id=claim.id, tenant_id=tenant_id, equipment_id=claim.equipment_id),
                       aggregate_id=claim.id, aggregate_type="Claim", tenant_id=tenant_id)
    ...
    def _transition(self, claim_id, new_status, *, mutate=None, extra_events=None) -> Claim:
        ...
        for factory in extra_events or []:
            self._emit(factory(claim, previous), aggregate_id=claim.id, aggregate_type="Claim", tenant_id=tenant_id)
        self._session.commit()
        self._session.refresh(claim)
        return claim

    def submit_claim_for_review(self, claim_id):
        def _m(c): c.reviewed_at = utcnow()
        return self._transition(
            claim_id, ClaimStatus.UNDER_REVIEW, mutate=_m,
            extra_events=[lambda c, prev: ClaimSubmittedForReview(claim_id=c.id, tenant_id=c.tenant_id, previous_status=prev.value)],
        )
    ...
    def approve_claim(self, claim_id, *, approved_amount, allow_override=False):
        ...
        if claim.claimed_amount is not None and Decimal(str(approved_amount)) > claim.claimed_amount and not allow_override:
            raise ValidationError("approved_amount cannot exceed claimed_amount without an authorized override.")
    ...
    def reject_claim(self, claim_id, *, reason):
        ...
        return self._transition(
            claim_id, ClaimStatus.REJECTED, mutate=_m,
            extra_events=[lambda c, prev: ClaimRejected(claim_id=c.id, tenant_id=c.tenant_id, previous_status=prev.value, reason=reason)],
        )
    ...
    def close_claim(self, claim_id):
        def _m(c): c.closed_at = utcnow()
        return self._transition(
            claim_id, ClaimStatus.CLOSED, mutate=_m,
            extra_events=[lambda c, prev: ClaimClosed(claim_id=c.id, tenant_id=c.tenant_id, previous_status=prev.value)],
        )

    def reopen_claim(self, claim_id, *, reason=None):
        def _m(c): c.reopened_at = utcnow()
        return self._transition(
            claim_id, ClaimStatus.UNDER_REVIEW, mutate=_m,
            extra_events=[lambda c, prev: ClaimReopened(claim_id=c.id, tenant_id=c.tenant_id, previous_status=prev.value, reason=reason)],
        )
    ...
    def list_claims(self, params) -> Page[Claim]:
        items, total = self._claims.list_claims(
            q=params.q, status=params.status, claim_type=params.claim_type,
            shipment_id=params.shipment_id, equipment_id=params.equipment_id, policy_id=params.policy_id,
    ...
    def delete_claim(self, claim_id) -> None:
        ...
        self._session.flush()
        self._emit(ClaimDeleted(claim_id=claim.id, tenant_id=tenant_id, deleted_by=actor_id),
                   aggregate_id=claim.id, aggregate_type="Claim", tenant_id=tenant_id)
        self._session.commit()
    ...
    def restore_claim(self, claim_id) -> Claim:
        ...
        self._session.flush()
        self._emit(ClaimRestored(claim_id=claim.id, tenant_id=tenant_id),
                   aggregate_id=claim.id, aggregate_type="Claim", tenant_id=tenant_id)
        self._session.commit()
        self._session.refresh(claim)
        return claim
    ...
    def create_damage_report(self, claim_id, **data) -> DamageReport:
        ...
        report = self._damage.create(
            tenant_id=tenant_id, claim_id=claim_id, reported_at=data.pop("reported_at", None) or utcnow(),
            created_by=actor_id, updated_by=actor_id, **data,
        )
        self._session.flush()
        self._emit(
            DamageReportCreated(damage_report_id=report.id, tenant_id=tenant_id, claim_id=claim_id,
                                damage_type=self._ev(report.damage_type)),
            aggregate_id=report.id, aggregate_type="DamageReport", tenant_id=tenant_id,
        )
        self._session.commit()
    ...
    def create_liability_record(self, claim_id, *, allow_override=False, **data) -> LiabilityRecord:
        ...
        actor_id = self._actor_id()
        self._load_claim_owned(claim_id, tenant_id)
        self.validate_liability_distribution(claim_id, data.get("liability_percentage"), allow_override=allow_override)
        record = self._liability.create(
            tenant_id=tenant_id, claim_id=claim_id, determined_at=data.pop("determined_at", None) or utcnow(),
            determined_by=actor_id, created_by=actor_id, updated_by=actor_id, **data,
        )
        self._session.flush()
        self._emit(
            LiabilityRecordCreated(liability_record_id=record.id, tenant_id=tenant_id, claim_id=claim_id,
                                   responsible_party_type=self._ev(record.responsible_party_type),
                                   liability_percentage=str(record.liability_percentage) if record.liability_percentage is not None else None),
            aggregate_id=record.id, aggregate_type="LiabilityRecord", tenant_id=tenant_id,
        )
        self._session.commit()
