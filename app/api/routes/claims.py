...
def _page(p) -> Page[ClaimRead]:
    return Page[ClaimRead](
        items=[ClaimRead.model_validate(x) for x in p.items],
        total=p.total,
        page=p.page,
        size=p.size,
        pages=p.pages,
...
@router.post(
    "", response_model=ClaimRead, status_code=status.HTTP_201_CREATED, summary="Create a claim."
)
def create_claim(
    payload: ClaimCreate,
...
def get_claim(
    ...
    return ClaimRead.model_validate(
        ClaimsService(session).get_claim(claim_id, include_deleted=include_deleted)
    )
...
def update_claim(
    ...
    return ClaimRead.model_validate(
        ClaimsService(session).update_claim(claim_id, **payload.model_dump(exclude_unset=True))
    )


@router.delete(
    "/{claim_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Soft-delete a claim."
)
def delete_claim(
    claim_id: uuid.UUID,
    session: Session = Depends(get_session),
...
def approve_claim(
    ...
    return ClaimRead.model_validate(
        ClaimsService(session).approve_claim(
            claim_id, approved_amount=payload.approved_amount, allow_override=allow
        )
    )
...
def reject_claim(
    ...
    return ClaimRead.model_validate(
        ClaimsService(session).reject_claim(claim_id, reason=payload.reason)
    )
...
def settle_claim(
    ...
    return ClaimRead.model_validate(
        ClaimsService(session).settle_claim(claim_id, settlement_notes=payload.settlement_notes)
    )
...
def reopen_claim(
    ...
    return ClaimRead.model_validate(
        ClaimsService(session).reopen_claim(claim_id, reason=payload.reason)
    )
...
@router.post(
    "/{claim_id}/damage-reports",
    response_model=DamageReportRead,
...
def create_damage_report(
    ...
    return DamageReportRead.model_validate(
        ClaimsService(session).create_damage_report(claim_id, **payload.model_dump())
    )


@router.get(
    "/{claim_id}/damage-reports",
    response_model=list[DamageReportRead],
...
def list_damage_reports(
    ...
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_READ)),
) -> list[DamageReportRead]:
    return [
        DamageReportRead.model_validate(x)
        for x in ClaimsService(session).list_damage_reports(claim_id)
    ]
...
@router.post(
    "/{claim_id}/liability-records",
    response_model=LiabilityRecordRead,
    status_code=status.HTTP_201_CREATED,
    summary="Add a liability record to a claim.",
)
def create_liability_record(
    claim_id: uuid.UUID,
    payload: LiabilityRecordCreate,
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_WRITE)),
) -> LiabilityRecordRead:
    ...
    return LiabilityRecordRead.model_validate(
        ClaimsService(session).create_liability_record(
            claim_id, allow_override=allow_override, **data
        )
    )
...
@router.get(
    "/{claim_id}/liability-records",
    response_model=list[LiabilityRecordRead],
    summary="List liability records.",
)
def list_liability_records(
    claim_id: uuid.UUID,
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_READ)),
) -> list[LiabilityRecordRead]:
    return [
        LiabilityRecordRead.model_validate(x)
        for x in ClaimsService(session).list_liability_records(claim_id)
    ]
