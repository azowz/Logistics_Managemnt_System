...
@router.get(
    "/dashboard",
    response_model=Optional[DashboardSummaryRead],
    summary="Tenant operations dashboard summary.",
)
def get_dashboard(
    session: Session = Depends(get_session), current_user=Depends(require_roles(*_READ))
) -> Optional[DashboardSummaryRead]:
...
@router.get(
    ...
    response_model=list[ShipmentPerformanceRead],
...
@router.get(
    "/financial/ar-aging", response_model=list[ARAgingRead], summary="Accounts-receivable aging."
)
def ar_aging(
...
@router.get(
    "/claims/metrics", response_model=list[ClaimsMetricsRead], summary="Claims metrics by period."
)
def claims_metrics(
    start_date: Optional[date] = Query(default=None),
    end_date: Optional[date] = Query(default=None),
...
@router.get(
    "/compliance/metrics",
    response_model=list[ComplianceMetricsRead],
    summary="Compliance metrics by period.",
)
...
@router.get(
    ...
    response_model=list[NotificationDeliverabilityRead],
    summary="Notification deliverability by period.",
)
def notification_deliverability(
    start_date: Optional[date] = Query(default=None),
    end_date: Optional[date] = Query(default=None),
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_READ)),
) -> list[NotificationDeliverabilityRead]:
    _validate_range(start_date, end_date)
    rows = ProjectionService(session).get_notification_deliverability(
...
@router.get(
    "/projections/health", response_model=list[ProjectionHealthRead], summary="Projection health."
)
def projection_health(
    session: Session = Depends(get_session), current_user=Depends(require_roles(*_READ))
) -> list[ProjectionHealthRead]:
    return [
        ProjectionHealthRead.model_validate(r)
        for r in ProjectionService(session).get_projection_health()
    ]
...
def rebuild_all(
    ...
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_REBUILD)),
) -> ProjectionRebuildResult:
    return ProjectionRebuildResult(
        **ProjectionService(session).rebuild_all_projections(dry_run=payload.dry_run)
    )


@router.post(
    "/projections/rebuild/{projection_type}",
    response_model=ProjectionRebuildResult,
...
def rebuild_one(
    ...
    return ProjectionRebuildResult(
        **ProjectionService(session).rebuild_projection(projection_type, dry_run=payload.dry_run)
    )
