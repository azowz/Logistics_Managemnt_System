...
def _tpl_page(p) -> Page[NotificationTemplateRead]:
    return Page[NotificationTemplateRead](
        items=[NotificationTemplateRead.model_validate(x) for x in p.items],
        total=p.total,
        page=p.page,
        size=p.size,
        pages=p.pages,
...
def _ntf_page(p) -> Page[NotificationRead]:
    return Page[NotificationRead](
        items=[NotificationRead.model_validate(x) for x in p.items],
        total=p.total,
        page=p.page,
        size=p.size,
        pages=p.pages,
...
def create_template(
    ...
    return NotificationTemplateRead.model_validate(
        NotificationService(session).create_template(**payload.model_dump())
    )


@router.get(
    "/templates/search", response_model=Page[NotificationTemplateRead], summary="Search templates."
)
...
@router.get(
    "/templates/{template_id}", response_model=NotificationTemplateRead, summary="Get a template."
)
def get_template(
    ...
    return NotificationTemplateRead.model_validate(
        NotificationService(session).get_template(template_id, include_deleted=include_deleted)
    )
...
def update_template(
    ...
    return NotificationTemplateRead.model_validate(
        NotificationService(session).update_template(
...
def restore_template(
    ...
    return NotificationTemplateRead.model_validate(
        NotificationService(session).restore_template(template_id)
    )
...
def activate_template(
    ...
    return NotificationTemplateRead.model_validate(
        NotificationService(session).activate_template(template_id)
    )
...
def deactivate_template(
    ...
    return NotificationTemplateRead.model_validate(
        NotificationService(session).deactivate_template(template_id)
    )
...
def create_notification(
    ...
    return NotificationRead.model_validate(
        NotificationService(session).create_notification(**payload.model_dump(by_alias=False))
    )
...
def list_unread(
    ...
    return _ntf_page(
        NotificationService(session).list_unread(current_user.id, page=page, size=size)
    )
...
def get_notification(
    ...
    return NotificationRead.model_validate(
        NotificationService(session).get_notification(
            notification_id,
            include_deleted=include_deleted,
            viewer_user_id=_viewer_scope(current_user),
        )
    )


@router.post(
    "/{notification_id}/queue", response_model=NotificationRead, summary="Queue a notification."
)
def queue_notification(
    ...
    return NotificationRead.model_validate(
        NotificationService(session).queue_notification(notification_id)
    )


@router.post(
    "/{notification_id}/send", response_model=NotificationRead, summary="Send a notification."
)
def send_notification(
    ...
    return NotificationRead.model_validate(
        NotificationService(session).send_notification(notification_id)
    )
...
def retry_notification(
    ...
    return NotificationRead.model_validate(
        NotificationService(session).retry_notification(notification_id)
    )


@router.post(
    "/{notification_id}/cancel", response_model=NotificationRead, summary="Cancel a notification."
)
def cancel_notification(
    ...
    return NotificationRead.model_validate(
        NotificationService(session).cancel_notification(notification_id, reason=payload.reason)
    )


@router.post(
    "/{notification_id}/read", response_model=NotificationRead, summary="Mark a notification read."
)
...
def list_attempts(
    ...
    return [
        NotificationDeliveryAttemptRead.model_validate(x)
        for x in NotificationService(session).list_delivery_attempts(notification_id)
    ]
