...
def _quote_page(p) -> Page[QuoteRead]:
    return Page[QuoteRead](
        items=[QuoteRead.model_validate(x) for x in p.items],
        total=p.total,
        page=p.page,
        size=p.size,
        pages=p.pages,
...
def _invoice_page(p) -> Page[InvoiceRead]:
    return Page[InvoiceRead](
        items=[InvoiceRead.model_validate(x) for x in p.items],
        total=p.total,
        page=p.page,
        size=p.size,
        pages=p.pages,
...
def _settlement_page(p) -> Page[SettlementRead]:
    return Page[SettlementRead](
        items=[SettlementRead.model_validate(x) for x in p.items],
        total=p.total,
        page=p.page,
        size=p.size,
        pages=p.pages,
...
def _penalty_page(p) -> Page[PenaltyRead]:
    return Page[PenaltyRead](
        items=[PenaltyRead.model_validate(x) for x in p.items],
        total=p.total,
        page=p.page,
        size=p.size,
        pages=p.pages,
...
def get_quote(
    ...
    return QuoteRead.model_validate(
        BillingService(session).get_quote(quote_id, include_deleted=include_deleted)
    )
...
def update_quote(
    ...
    return QuoteRead.model_validate(
        BillingService(session).update_quote(quote_id, **payload.model_dump(exclude_unset=True))
    )
...
def reject_quote(
    ...
    return QuoteRead.model_validate(
        BillingService(session).reject_quote(quote_id, reason=payload.reason)
    )
...
def cancel_quote(
    ...
    return QuoteRead.model_validate(
        BillingService(session).cancel_quote(quote_id, reason=payload.reason)
    )
...
def get_invoice(
    ...
    return InvoiceRead.model_validate(
        BillingService(session).get_invoice(invoice_id, include_deleted=include_deleted)
    )


@router.patch(
    "/invoices/{invoice_id}", response_model=InvoiceRead, summary="Update a draft invoice."
)
def update_invoice(
    invoice_id: uuid.UUID,
    ...
    return InvoiceRead.model_validate(
        BillingService(session).update_invoice(invoice_id, **payload.model_dump(exclude_unset=True))
    )


@router.get(
    "/invoices/{invoice_id}/lines",
    response_model=list[InvoiceLineRead],
...
def list_invoice_lines(
    ...
    return [
        InvoiceLineRead.model_validate(x)
        for x in BillingService(session).list_invoice_lines(invoice_id)
    ]


@router.post(
    "/invoices/{invoice_id}/issue", response_model=InvoiceRead, summary="Issue an invoice."
)
def issue_invoice(
    invoice_id: uuid.UUID,
    session: Session = Depends(get_session),
...
def record_payment(
    ...
    return PaymentRead.model_validate(
        BillingService(session).record_payment(
            invoice_id,
...
def list_payments(
    ...
    return [
        PaymentRead.model_validate(x) for x in BillingService(session).list_payments(invoice_id)
    ]
...
def void_invoice(
    ...
    allow = payload.allow_override and current_user.role == UserRole.ADMIN
    return InvoiceRead.model_validate(
        BillingService(session).void_invoice(
            invoice_id, reason=payload.reason, allow_override=allow
        )
    )


@router.post(
    "/invoices/{invoice_id}/cancel", response_model=InvoiceRead, summary="Cancel an invoice."
)
def cancel_invoice(
    invoice_id: uuid.UUID,
    ...
    return InvoiceRead.model_validate(
        BillingService(session).cancel_invoice(invoice_id, reason=payload.reason)
    )
...
def create_settlement(
    ...
    return SettlementRead.model_validate(
        SettlementService(session).create_settlement(allow_override=allow, **data)
    )


@router.get(
    "/settlements/search", response_model=Page[SettlementRead], summary="Search settlements."
)
...
@router.get(
    "/settlements/{settlement_id}", response_model=SettlementRead, summary="Get a settlement."
)
def get_settlement(
    ...
    return SettlementRead.model_validate(
        SettlementService(session).get_settlement(settlement_id, include_deleted=include_deleted)
    )


@router.patch(
    "/settlements/{settlement_id}", response_model=SettlementRead, summary="Update a settlement."
)
def update_settlement(
    ...
    return SettlementRead.model_validate(
        SettlementService(session).update_settlement(
            settlement_id, **payload.model_dump(exclude_unset=True)
        )
    )
...
@router.post(
    ...
    response_model=SettlementRead,
    summary="Submit a settlement for approval.",
)
def submit_settlement(
    ...
    return SettlementRead.model_validate(
        SettlementService(session).submit_settlement_for_approval(settlement_id)
    )
...
def approve_settlement(
    ...
    return SettlementRead.model_validate(
        SettlementService(session).approve_settlement(settlement_id)
    )
...
def settle_settlement(
    ...
    return SettlementRead.model_validate(
        SettlementService(session).settle_settlement(settlement_id)
    )
...
def cancel_settlement(
    ...
    return SettlementRead.model_validate(
        SettlementService(session).cancel_settlement(settlement_id, reason=payload.reason)
    )


@router.post(
    "/settlements/{settlement_id}/payouts",
...
def create_payout(
    ...
    return PayoutRead.model_validate(
        SettlementService(session).create_payout(
            settlement_id,
            amount=payload.amount,
            method=payload.method,
...
@router.get(
    ...
    summary="List payouts for a settlement.",
)
def list_payouts(
    settlement_id: uuid.UUID,
    session: Session = Depends(get_session),
    ...
    return [
        PayoutRead.model_validate(x) for x in SettlementService(session).list_payouts(settlement_id)
    ]
