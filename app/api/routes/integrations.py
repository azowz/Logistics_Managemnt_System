...
@router.post(
    "/partners",
    response_model=IntegrationPartnerRead,
    status_code=status.HTTP_201_CREATED,
    summary="Create an integration partner.",
)
def create_partner(
    payload: IntegrationPartnerCreate,
...
@router.get(
    "/partners", response_model=list[IntegrationPartnerRead], summary="List integration partners."
)
def list_partners(
    params: PartnerListParams = Depends(),
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_MANAGE)),
) -> list[IntegrationPartnerRead]:
...
@router.get(
    "/partners/search",
    response_model=list[IntegrationPartnerRead],
    summary="Search integration partners.",
)
def search_partners(
    params: PartnerListParams = Depends(),
    session: Session = Depends(get_session),
...
@router.get(
    "/partners/{partner_id}",
    response_model=IntegrationPartnerRead,
    summary="Get an integration partner.",
)
def get_partner(
    ...
    return IntegrationPartnerRead.model_validate(
        IntegrationService(session).get_partner(partner_id)
    )
...
@router.patch(
    "/partners/{partner_id}",
    response_model=IntegrationPartnerRead,
    summary="Update an integration partner.",
)
def update_partner(
    partner_id: uuid.UUID,
    payload: IntegrationPartnerUpdate,
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_MANAGE)),
) -> IntegrationPartnerRead:
    partner = IntegrationService(session).update_partner(
        partner_id, **payload.model_dump(exclude_unset=True)
    )
    return IntegrationPartnerRead.model_validate(partner)
...
def activate_partner(
    ...
    return IntegrationPartnerRead.model_validate(
        IntegrationService(session).activate_partner(partner_id)
    )
...
def suspend_partner(
    ...
    return IntegrationPartnerRead.model_validate(
        IntegrationService(session).suspend_partner(partner_id)
    )
...
@router.post(
    ...
    response_model=PartnerApiKeyCreatedRead,
    status_code=status.HTTP_201_CREATED,
    summary="Create a partner API key (plaintext shown once).",
)
def create_api_key(
    partner_id: uuid.UUID,
...
@router.get(
    "/partners/{partner_id}/api-keys",
    response_model=list[PartnerApiKeyRead],
...
def list_api_keys(
    ...
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_MANAGE)),
) -> list[PartnerApiKeyRead]:
    return [
        PartnerApiKeyRead.model_validate(k)
        for k in IntegrationService(session).list_api_keys(partner_id)
    ]


@router.post(
    "/api-keys/{api_key_id}/revoke",
    response_model=PartnerApiKeyRead,
...
@router.post(
    ...
    response_model=WebhookSubscriptionCreatedRead,
    status_code=status.HTTP_201_CREATED,
...
def get_subscription(
    ...
    return WebhookSubscriptionRead.model_validate(
        IntegrationService(session).get_subscription(subscription_id)
    )
...
def update_subscription(
    ...
    sub = IntegrationService(session).update_subscription(
        subscription_id, **payload.model_dump(exclude_unset=True)
    )
    return WebhookSubscriptionRead.model_validate(sub)


@router.post(
    "/webhooks/subscriptions/{subscription_id}/activate",
    response_model=WebhookSubscriptionRead,
...
def activate_subscription(
    ...
    return WebhookSubscriptionRead.model_validate(
        IntegrationService(session).activate_subscription(subscription_id)
    )


@router.post(
    "/webhooks/subscriptions/{subscription_id}/deactivate",
    response_model=WebhookSubscriptionRead,
...
def deactivate_subscription(
    ...
    return WebhookSubscriptionRead.model_validate(
        IntegrationService(session).deactivate_subscription(subscription_id)
    )
...
def list_deliveries(
    params: DeliveryListParams = Depends(),
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_MANAGE)),
    ...
    items, _ = IntegrationService(session).list_deliveries(
        ...
        partner_id=params.partner_id,
        status=params.status,
        external_event_type=params.external_event_type,
        sort_by=params.sort_by,
        sort_dir=params.sort_dir,
        limit=params.size,
        offset=params.offset,
...
def retry_delivery(
    ...
    return WebhookDeliveryRead.model_validate(
        IntegrationService(session).retry_delivery(delivery_id)
    )


@router.post(
    "/webhooks/deliveries/{delivery_id}/cancel",
    response_model=WebhookDeliveryRead,
...
def cancel_delivery(
    ...
    return WebhookDeliveryRead.model_validate(
        IntegrationService(session).cancel_delivery(delivery_id)
    )


@router.get(
    "/webhooks/deliveries/{delivery_id}/attempts",
    response_model=list[WebhookDeliveryAttemptRead],
...
def list_delivery_attempts(
    ...
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_MANAGE)),
) -> list[WebhookDeliveryAttemptRead]:
    return [
        WebhookDeliveryAttemptRead.model_validate(a)
...
@router.post(
    "/inbound/events",
    response_model=InboundIntegrationEventRead,
    status_code=status.HTTP_201_CREATED,
...
async def receive_inbound_event(
    ...
    signature_valid = bool(
        x_mesaar_signature
        and crypto.verify_signature(
            principal.api_key, raw, x_mesaar_signature, timestamp=x_mesaar_timestamp
        )
    )
    row = IntegrationService(session).receive_inbound_event(
        partner_id=principal.context.partner_id,
        api_key_id=principal.context.api_key_id,
