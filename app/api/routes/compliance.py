...
def create_permit(
    ...
    return PermitRead.model_validate(
        ComplianceService(session).create_permit(**payload.model_dump())
    )
...
def _permit_page(p) -> Page[PermitRead]:
    return Page[PermitRead](
        items=[PermitRead.model_validate(x) for x in p.items],
        total=p.total,
        page=p.page,
        size=p.size,
        pages=p.pages,
...
def get_permit(
    ...
    return PermitRead.model_validate(
        ComplianceService(session).get_permit(permit_id, include_deleted=include_deleted)
    )
...
def update_permit(
    ...
    return PermitRead.model_validate(
        ComplianceService(session).update_permit(
            permit_id, **payload.model_dump(exclude_unset=True)
        )
    )


@router.delete(
    "/permits/{permit_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Soft-delete a permit."
)
def delete_permit(
    permit_id: uuid.UUID,
    session: Session = Depends(get_session),
...
@router.post(
    "/permits/{permit_id}/review", response_model=PermitRead, summary="Mark a permit under review."
)
def review_permit(
    permit_id: uuid.UUID,
    session: Session = Depends(get_session),
...
def approve_permit(
    permit_id: uuid.UUID,
    payload: PermitStatusRequest,
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_WRITE)),
) -> PermitRead:
    return PermitRead.model_validate(
        ComplianceService(session).approve_permit(
            permit_id, valid_from=payload.valid_from, valid_until=payload.valid_until
        )
    )
...
def reject_permit(
    ...
    return PermitRead.model_validate(
        ComplianceService(session).reject_permit(permit_id, reason=payload.reason)
    )


@router.post(
    "/permits/{permit_id}/activate", response_model=PermitRead, summary="Activate a permit."
)
def activate_permit(
    permit_id: uuid.UUID,
    session: Session = Depends(get_session),
...
def cancel_permit(
    ...
    return PermitRead.model_validate(
        ComplianceService(session).cancel_permit(permit_id, reason=payload.reason)
    )
...
def list_checks(
    ...
    return Page[ComplianceCheckRead](
        items=[ComplianceCheckRead.model_validate(x) for x in p.items],
        total=p.total,
        page=p.page,
        size=p.size,
        pages=p.pages,
...
@router.get(
    "/checks/{check_id}", response_model=ComplianceCheckRead, summary="Get a compliance check."
)
def get_check(
    check_id: uuid.UUID,
    session: Session = Depends(get_session),
...
@router.post(
    ...
    summary="Override a compliance check.",
)
def override_check(
    check_id: uuid.UUID,
    payload: ComplianceOverrideRequest,
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_OVERRIDE)),
) -> ComplianceCheckRead:
    return ComplianceCheckRead.model_validate(
        ComplianceService(session).apply_compliance_override(check_id, reason=payload.reason)
    )
...
@router.post(
    "/escorts",
    response_model=EscortRead,
    status_code=status.HTTP_201_CREATED,
    summary="Create an escort.",
)
def create_escort(
    payload: EscortCreate,
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_WRITE)),
) -> EscortRead:
    return EscortRead.model_validate(
        ComplianceService(session).create_escort(**payload.model_dump())
    )
...
def list_escorts(
    ...
    return Page[EscortRead](
        items=[EscortRead.model_validate(x) for x in p.items],
        total=p.total,
        page=p.page,
        size=p.size,
        pages=p.pages,
...
@router.post(
    "/escorts/{escort_id}/schedule", response_model=EscortRead, summary="Schedule an escort."
)
def schedule_escort(
    escort_id: uuid.UUID,
    payload: EscortScheduleRequest,
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_WRITE)),
) -> EscortRead:
    return EscortRead.model_validate(
        ComplianceService(session).schedule_escort(
            escort_id, scheduled_start=payload.scheduled_start, scheduled_end=payload.scheduled_end
        )
    )
...
@router.post(
    "/route-restrictions",
    response_model=RouteRestrictionRead,
    status_code=status.HTTP_201_CREATED,
    summary="Create a route restriction.",
)
def create_restriction(
    payload: RouteRestrictionCreate,
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_WRITE)),
) -> RouteRestrictionRead:
    return RouteRestrictionRead.model_validate(
        ComplianceService(session).create_route_restriction(**payload.model_dump())
    )


@router.get(
    "/route-restrictions",
    response_model=list[RouteRestrictionRead],
...
@router.patch(
    "/route-restrictions/{restriction_id}",
    response_model=RouteRestrictionRead,
    summary="Update a route restriction.",
)
def update_restriction(
    restriction_id: uuid.UUID,
    payload: RouteRestrictionUpdate,
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_WRITE)),
) -> RouteRestrictionRead:
    return RouteRestrictionRead.model_validate(
        ComplianceService(session).update_route_restriction(
            restriction_id, **payload.model_dump(exclude_unset=True)
        )
    )
...
@router.post(
    "/axle-weight-profiles",
    response_model=AxleWeightProfileRead,
    status_code=status.HTTP_201_CREATED,
    summary="Create an axle-weight profile.",
)
def create_axle_profile(
    payload: AxleWeightProfileCreate,
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_WRITE)),
) -> AxleWeightProfileRead:
    return AxleWeightProfileRead.model_validate(
        ComplianceService(session).create_axle_weight_profile(**payload.model_dump())
    )
...
@router.post(
    "/operator-certifications",
    response_model=OperatorCertificationRead,
    status_code=status.HTTP_201_CREATED,
    summary="Create an operator certification.",
)
def create_certification(
    payload: OperatorCertificationCreate,
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_WRITE)),
) -> OperatorCertificationRead:
    return OperatorCertificationRead.model_validate(
        ComplianceService(session).create_operator_certification(**payload.model_dump())
    )


@router.get(
    "/operator-certifications",
    response_model=list[OperatorCertificationRead],
...
def list_certifications(
    ...
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_READ)),
) -> list[OperatorCertificationRead]:
...
@router.post(
    "/operator-certifications/{certification_id}/expire",
    response_model=OperatorCertificationRead,
    summary="Expire an operator certification.",
)
def expire_certification(
    certification_id: uuid.UUID,
    session: Session = Depends(get_session),
    current_user=Depends(require_roles(*_WRITE)),
) -> OperatorCertificationRead:
    return OperatorCertificationRead.model_validate(
        ComplianceService(session).expire_operator_certification(certification_id)
    )
