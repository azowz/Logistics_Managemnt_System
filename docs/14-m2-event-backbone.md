...
> **Status:** Implemented (M2). Final self-review per the M2 brief. Date 2026-06-22.
...
| Event store + transactional outbox | `app/models/event_store.py`, migration `0005` | append-only; `UNIQUE(aggregate_id, aggregate_version)`; outbox columns (`published_at`, `publish_attempts`, `last_error`, `next_attempt_at`); partial outbox index |
| Idempotency ledger | `processed_events` (`(consumer, event_id)`) | effectively-once consumers |
| Dead-letter queue | `dead_letter_events` | reason, retry count, replay-ready, tenant-scoped |
| Relay cursor / lag | `outbox_relay_state` + `OUTBOX_DEPTH` gauge | observability |
| Repository | `app/repositories/event_store_repository.py` | append(+audit), outbox poll, mark-published, publish-failure, dead-letter, replay (aggregate/tenant/date/type), idempotency |
...
| Async relay (outbox→bus) | `app/events/relay.py` + `mesaar.relay_outbox` task | per-event tenant-scoped transaction; transport retry + DLQ |
...
Concrete domain events and the shipment/driver/fleet/warehouse/analytics projections are owned by their bounded contexts and land with their milestones (M3+); this backbone is the foundation they plug into.
...
| **Idempotent processing** | ✅ | `processed_events (consumer, event_id)`; dispatcher skips already-processed; consumers also recommended idempotent |
...
| **Alembic-safe / additive** | ✅ | `0004`/`0005` additive; `Base.metadata` + `app.models` unchanged as targets; constraint naming convention honored (fixed a pre-existing CheckConstraint double-prefix) |
...
**Conclusion:** the backbone is tenant-isolating by design and in code; the two residual security conditions (non-superuser role + CI RLS verification) are the same M1 production gates from `docs/10`, now extended to the event tables.
...
## 7. File Tree (M2 additions / changes)

```
app/
  events/                         # NEW — event backbone
    __init__.py  domain_event.py  envelope.py  registry.py
    exceptions.py  metrics.py  bus.py  dispatcher.py  relay.py
  ...
  repositories/
    event_store_repository.py     # NEW
  db/tenant.py  db/session.py     # CHANGED — current-user context + dual GUC listener
  core/security.py  auth/tokens.py services/auth_service.py services/exceptions.py  # CHANGED — user/tid claim + ConflictError
  api/middleware/request_context.py  # CHANGED — user-context lifecycle
...
tests/
  ...
  test_event_bus.py test_event_models_metadata.py test_event_backbone_pg.py
  test_session_guc.py (updated)   # + existing M1 tests
```
...
**`31 passed, 9 skipped`** (skips are the Postgres-gated suites, run in CI).
...
| `test_event_backbone_pg` | **integration (PG-gated)** | append+audit, **optimistic-concurrency conflict**, **outbox relay end-to-end + idempotency**, **replay ordering**, **RLS isolation** |
...
| Optimistic `version_id_col` runtime behavior | config-asserted here (no PG); concurrency conflict covered by the PG suite |
...
**🟢 M2 Enterprise Event Backbone delivered** — append-only event store, transactional outbox (no dual-write), idempotent dispatch, retries + dead-letter, pluggable bus, projection rebuild, audit integration, replay, versioning/upcasting, observability, and a full test suite (DB-free unit + PG-gated integration). It complies with the approved architecture; the only open items are the **build/CI production conditions R-1/R-2** (non-superuser role + RLS verification) carried from the database freeze, plus the deliberately deferred native-partitioning ADR. No known architectural debt was introduced.

*Companion: `docs/13-m2-readiness-report.md` (gate), `docs/11-backend-architecture.md` §6 (design), ADR-004/007.*
