...
| --- | --- | --- |
| ~~Metrics not present in the event stream are stored as 0: `on_time/late_deliveries`, `average_delivery_duration_minutes`, `total_claimed_amount` & `average_claim_cycle_days`, `unread_urgent_notifications`.~~ | ~~MEDIUM~~ | **RESOLVED in Sprint 12 (docs/26):** the source events were enriched (additive, v1-compatible) and the projection handlers now populate every one of these. Historical events without the new keys still apply (the fields default to `None` and simply don't contribute). |
| ~~`proj_financial_summary` is single-currency (`SAR`): billing events omit `currency_code`.~~ | ~~MEDIUM~~ | **RESOLVED in Sprint 12 (docs/26):** billing/claims events now carry `currency_code`; financial & claims projections key on the event's currency (legacy payloads collapse to `SAR`), and the analytics API exposes a `currency_code` filter. |
